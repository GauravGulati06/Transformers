...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1) # (seq_len, 1) represent the position of word in the sentence

        div_term = torch.exp(torch.arange(0, d_model, 2, dtype=torch.float) * (-math.log(10000.0) / d_model))

        # pe[words, dimension] -> sin() on even indexed embeddings "YOUR" word -> PE[0,0]
        pe[:, 0::2] = torch.sin(position * div_term)
        # cos() on odd indexed embeddings -> "YOUR" -> PE[0,1] till PE[0,512] of "YOUR" word
    ...
    def forward(self, x):
        x = x + self.pe[:, :x.shape[1], :].requires_grad_(False) # False becuz we dont our model to train these as parameters they are saved 
...
class LayerNormalization(nn.Module):
    ...
    def __init__(self, eps: float = 1e-5):
        super().__init__()
        self.eps = eps
        self.alpha = nn.Parameter(torch.ones(1)) # act as weight i.e. multiplied
        self.beta = nn.Parameter(torch.zeros(1)) # act as bias i.e. added
